 ...
 * A list of mere in EXTMA1 and not in EXTMA2
 ...
 ddecosterd@hetic3.fr    2026-04-10       1.1              Limit number of read record to 10000. Add an in parameter "mere" for start value. Add an out parameter to know if we have reaad all the records. 
 ******************************************************************************************/
	...
	public void main() {
		...
		while(readExtma1 != 0 && nrOfRecords < nrMaxOfRecords) {
			...
			if(nrOfRecords < nrMaxOfRecords) {
				extmat1ExpressionFactory = database.getExpressionFactory("EXTMA1");
				extmat1ExpressionFactory = extmat1ExpressionFactory.gt("EXMERE", mere.toString());
