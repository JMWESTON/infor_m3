...
public class DelSCHN extends ExtendM3Transaction {
	...
	public DelSCHN(MIAPI mi, ProgramAPI program, DatabaseAPI database, UtilityAPI utility) {
		this.mi = mi;
	...
	public void main() {
		...
		if(!checkInputs(CONO, FACI, MFNO, PRNO, OPNO))
			return;
		...
		if(!updateMwoopeSCHN(mwoopeRecord, mwoopeContainer)) {
			mi.error("No record matching required criterias found.");
			return;
		}

		if(!MATHED(mwomatContainer)){
			mi.error("No record matching required criterias found.");
			return;
		}
	}
	 ...
	 */
	private boolean checkInputs(Integer cono, String  faci, String  mfno, String  prno, Integer opno) {
		if(cono == null) {
	...
	/**
	 * Update SCHN value to zero in MWOOPE for the selected lines.
	 ...
	 */
	private boolean updateMwoopeSCHN(DBAction mwoopeRecord, DBContainer mwoopeContainer) {
		...
		int pagesize = mi.getMaxRecords() <= 0 || mi.getMaxRecords() >= 10000 ? 10000: mi.getMaxRecords();
		return mwoopeRecord.readAll(mwoopeContainer,5,pagesize,mwoopeClosure)!=0;
	}

	/**
	 *    MATHED - Update material with schedulenumber
	 */
	private boolean MATHED(DBContainer mwomatContainer) {
